 ...
 */
@VisibleForTesting
class FastScroller extends RecyclerTmView.ItemDecoration implements RecyclerTmView.OnItemTouchListener {
    ...
    private final RecyclerTmView.OnScrollListener
            mOnScrollListener = new RecyclerTmView.OnScrollListener() {
        @Override
        public void onScrolled(RecyclerTmView recyclerTmView, int dx, int dy) {
            updateScrollPosition(recyclerTmView.computeHorizontalScrollOffset(),
                    recyclerTmView.computeVerticalScrollOffset());
        }
    };

    FastScroller(RecyclerTmView recyclerTmView, StateListDrawable verticalThumbDrawable,
        ...
        attachToRecyclerView(recyclerTmView);
    }
    ...
    private void setupCallbacks() {
        ...
        mRecyclerTmView.addOnItemTouchListener(this);
        mRecyclerTmView.addOnScrollListener(mOnScrollListener);
    }

    private void destroyCallbacks() {
        mRecyclerTmView.removeItemDecoration(this);
        mRecyclerTmView.removeOnItemTouchListener(this);
        mRecyclerTmView.removeOnScrollListener(mOnScrollListener);
        cancelHide();
    }
    ...
    private boolean isLayoutRTL() {
        return ViewCompat.getLayoutDirection(mRecyclerTmView) == ViewCompat.LAYOUT_DIRECTION_RTL;
    }
    ...
    private void cancelHide() {
        mRecyclerTmView.removeCallbacks(mHideRunnable);
    }

    private void resetHideDelay(int delay) {
        cancelHide();
        mRecyclerTmView.postDelayed(mHideRunnable, delay);
    }

    @Override
    public void onDrawOver(Canvas canvas, RecyclerTmView parent, RecyclerTmView.State state) {
        if (mRecyclerViewWidth != mRecyclerTmView.getWidth()
                || mRecyclerViewHeight != mRecyclerTmView.getHeight()) {
            mRecyclerViewWidth = mRecyclerTmView.getWidth();
            mRecyclerViewHeight = mRecyclerTmView.getHeight();
    ...
    void updateScrollPosition(int offsetX, int offsetY) {
        int verticalContentLength = mRecyclerTmView.computeVerticalScrollRange();
        int verticalVisibleLength = mRecyclerViewHeight;
        mNeedVerticalScrollbar = verticalContentLength - verticalVisibleLength > 0
            && mRecyclerViewHeight >= mScrollbarMinimumRange;

        int horizontalContentLength = mRecyclerTmView.computeHorizontalScrollRange();
    ...
    @Override
    public void onTouchEvent(@NonNull RecyclerTmView recyclerTmView, @NonNull MotionEvent me) {
        if (mState == STATE_HIDDEN) {
            return;
        }
    ...
    private void verticalScrollTo(float y) {
        ...
        int scrollingBy = scrollTo(mVerticalDragY, y, scrollbarRange,
                mRecyclerTmView.computeVerticalScrollRange(),
                mRecyclerTmView.computeVerticalScrollOffset(), mRecyclerViewHeight);
        if (scrollingBy != 0) {
            mRecyclerTmView.scrollBy(0, scrollingBy);
        }
        mVerticalDragY = y;
    }
    ...
    private void horizontalScrollTo(float x) {
        ...
        int scrollingBy = scrollTo(mHorizontalDragX, x, scrollbarRange,
                mRecyclerTmView.computeHorizontalScrollRange(),
                mRecyclerTmView.computeHorizontalScrollOffset(), mRecyclerViewWidth);
        if (scrollingBy != 0) {
            mRecyclerTmView.scrollBy(scrollingBy, 0);
        }
