...
/**
 * A {@link RecyclerTmView.LayoutManager} implementations that lays out items in a grid.
 * <p>
 * By default, each item occupies 1 span. You can change it by providing a custom
 * {@link SpanSizeLookup} instance via {@link #setSpanSizeLookup(SpanSizeLookup)}.
 */
public class GridLayoutManager extends LinearLayoutTmManager {
     ...
     */
    public GridLayoutManager(Context context, int spanCount,
                             @RecyclerTmView.Orientation int orientation, boolean reverseLayout) {
    ...
    private void ensureAnchorIsInCorrectSpan(RecyclerTmView.Recycler recycler,
                                             RecyclerTmView.State state, AnchorInfo anchorInfo, int itemDirection) {
    ...
    private int getSpanGroupIndex(RecyclerTmView.Recycler recycler, RecyclerTmView.State state,
                                  int viewPosition) {
        if (!state.isPreLayout()) {
    ...
    private int getSpanIndex(RecyclerTmView.Recycler recycler, RecyclerTmView.State state, int pos) {
        if (!state.isPreLayout()) {
    ...
    protected int getSpanSize(RecyclerTmView.Recycler recycler, RecyclerTmView.State state, int pos) {
        if (!state.isPreLayout()) {
    ...
    protected void assignSpans(RecyclerTmView.Recycler recycler, RecyclerTmView.State state, int count,
    ...
    public View onFocusSearchFailed(View focused, int focusDirection,
        ...
        // LinearLayoutTmManager finds the last child. What we want is the child which has the same
        // spanIndex.
    ...
    private int computeScrollRangeWithSpanInfo(RecyclerTmView.State state) {
    ...
    private int computeScrollOffsetWithSpanInfo(RecyclerTmView.State state) {
        if (getChildCount() == 0 || state.getItemCount() == 0) {
            return 0;
        }
        ensureLayoutState();
