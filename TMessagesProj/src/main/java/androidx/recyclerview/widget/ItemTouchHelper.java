...
import androidx.recyclerview.widget.RecyclerTmView.OnItemTouchListener;
import androidx.recyclerview.widget.RecyclerTmView.ViewHolder;
 ...
 * {@link Callback#onMove(RecyclerTmView, ViewHolder, ViewHolder)} and / or
 ...
 * customize these behaviors by overriding {@link Callback#onChildDraw(Canvas, RecyclerTmView,
 * ViewHolder, float, float, int, boolean)}
 * or {@link Callback#onChildDrawOver(Canvas, RecyclerTmView, ViewHolder, float, float, int,
 * boolean)}.
 * <p/>
 * Most of the time you only need to override <code>onChildDraw</code>.
 */
public class ItemTouchHelper extends RecyclerTmView.ItemDecoration
        implements RecyclerTmView.OnChildAttachStateChangeListener {
     ...
     * {@link Callback#getAbsoluteMovementFlags(RecyclerTmView, ViewHolder)} for the current
     * action state.
     */
    @SuppressWarnings("WeakerAccess") /* synthetic access */
    int mSelectedFlags;
    ...
    final Runnable mScrollRunnable = new Runnable() {
        ...
        public void run() {
            if (mSelected != null && scrollIfNecessary()) {
                ...
                mRecyclerTmView.removeCallbacks(mScrollRunnable);
                ViewCompat.postOnAnimation(mRecyclerTmView, this);
            }
        }
    };
     ...
     */
    private RecyclerTmView.ChildDrawingOrderCallback mChildDrawingOrderCallback = null;
    ...
    private final OnItemTouchListener mOnItemTouchListener = new OnItemTouchListener() {
        @Override
        public boolean onInterceptTouchEvent(@NonNull RecyclerTmView recyclerTmView,
                ...
                if (mSelected == null) {
                    ...
                    if (animation != null) {
                        ...
                        if (mPendingCleanup.remove(animation.mViewHolder.itemView)) {
                            mCallback.clearView(mRecyclerTmView, animation.mViewHolder);
                        }
        ...
        @Override
        public void onTouchEvent(@NonNull RecyclerTmView recyclerTmView, @NonNull MotionEvent event) {
            mGestureDetector.onTouchEvent(event);
            if (DEBUG) {
                Log.d(TAG,
     ...
     * {@link #attachToRecyclerView(RecyclerTmView)}. Upon attaching, it will add an item decoration,
    ...
    public void attachToRecyclerView(@Nullable RecyclerTmView recyclerTmView) {
        ...
        if (recyclerTmView != null) {
            final Resources resources = recyclerTmView.getResources();
    ...
    private void setupCallbacks() {
        ViewConfiguration vc = ViewConfiguration.get(mRecyclerTmView.getContext());
        mSlop = vc.getScaledTouchSlop();
        mRecyclerTmView.addItemDecoration(this);
        mRecyclerTmView.addOnItemTouchListener(mOnItemTouchListener);
        mRecyclerTmView.addOnChildAttachStateChangeListener(this);
        startGestureDetection();
    }

    private void destroyCallbacks() {
        mRecyclerTmView.removeItemDecoration(this);
        mRecyclerTmView.removeOnItemTouchListener(mOnItemTouchListener);
        mRecyclerTmView.removeOnChildAttachStateChangeListener(this);
        // clean all attached
        final int recoverAnimSize = mRecoverAnimations.size();
        for (int i = recoverAnimSize - 1; i >= 0; i--) {
            final RecoverAnimation recoverAnimation = mRecoverAnimations.get(0);
            mCallback.clearView(mRecyclerTmView, recoverAnimation.mViewHolder);
        }
    ...
    private void startGestureDetection() {
        ...
        mGestureDetector = new GestureDetectorCompat(mRecyclerTmView.getContext(),
    ...
    @Override
    public void onDrawOver(Canvas c, RecyclerTmView parent, RecyclerTmView.State state) {
        float dx = 0, dy = 0;
        if (mSelected != null) {
    ...
    @Override
    public void onDraw(Canvas c, RecyclerTmView parent, RecyclerTmView.State state) {
    ...
    void select(@Nullable ViewHolder selected, int actionState) {
        ...
        if (mSelected != null) {
            ...
            if (prevSelected.itemView.getParent() != null) {
                ...
                switch (swipeDir) {
                    ...
                    case END:
                        targetTranslateY = 0;
                        targetTranslateX = Math.signum(mDx) * mRecyclerTmView.getWidth();
                        break;
                    case UP:
                    case DOWN:
                        targetTranslateX = 0;
                        targetTranslateY = Math.signum(mDy) * mRecyclerTmView.getHeight();
                        break;
                    default:
                        targetTranslateX = 0;
                        targetTranslateY = 0;
                ...
                final RecoverAnimation rv = new RecoverAnimation(prevSelected, animationType,
                    ...
                    public void onAnimationEnd(Animator animation) {
                        ...
                        if (swipeDir <= 0) {
                            ...
                            mCallback.clearView(mRecyclerTmView, prevSelected);
                ...
                final long duration = mCallback.getAnimationDuration(mRecyclerTmView, animationType,
        ...
        if (selected != null) {
            mSelectedFlags =
                    (mCallback.getAbsoluteMovementFlags(mRecyclerTmView, selected) & actionStateMask)
        ...
        final ViewParent rvParent = mRecyclerTmView.getParent();
        if (rvParent != null) {
            rvParent.requestDisallowInterceptTouchEvent(mSelected != null);
        }
        if (!preventLayout) {
            mRecyclerTmView.getLayoutManager().requestSimpleAnimationsInNextLayout();
        }
    ...
    void postDispatchSwipe(final RecoverAnimation anim, final int swipeDir) {
        ...
        mRecyclerTmView.post(new Runnable() {
            @Override
            public void run() {
                if (mRecyclerTmView != null && mRecyclerTmView.isAttachedToWindow()
                        && !anim.mOverridden
                        && anim.mViewHolder.getAdapterPosition() != RecyclerTmView.NO_POSITION) {
                    final RecyclerTmView.ItemAnimator animator = mRecyclerTmView.getItemAnimator();
    ...
    boolean scrollIfNecessary() {
        ...
        RecyclerTmView.LayoutManager lm = mRecyclerTmView.getLayoutManager();
        if (mTmpRect == null) {
            mTmpRect = new Rect();
        }
        int scrollX = 0;
        ...
        if (lm.canScrollHorizontally()) {
            ...
            final int leftDiff = curX - mTmpRect.left - mRecyclerTmView.getPaddingLeft();
            if (mDx < 0 && leftDiff < 0) {
                scrollX = leftDiff;
            } else if (mDx > 0) {
                final int rightDiff =
                        curX + mSelected.itemView.getWidth() + mTmpRect.right
                                - (mRecyclerTmView.getWidth() - mRecyclerTmView.getPaddingRight());
                if (rightDiff > 0) {
                    scrollX = rightDiff;
                }
            }
        ...
        if (lm.canScrollVertically()) {
            ...
            final int topDiff = curY - mTmpRect.top - mRecyclerTmView.getPaddingTop();
            if (mDy < 0 && topDiff < 0) {
                scrollY = topDiff;
            } else if (mDy > 0) {
                final int bottomDiff = curY + mSelected.itemView.getHeight() + mTmpRect.bottom
                        - (mRecyclerTmView.getHeight() - mRecyclerTmView.getPaddingBottom());
                if (bottomDiff > 0) {
                    scrollY = bottomDiff;
                }
            }
        ...
        if (scrollX != 0) {
            scrollX = mCallback.interpolateOutOfBoundsScroll(mRecyclerTmView,
                    mSelected.itemView.getWidth(), scrollX,
                    mRecyclerTmView.getWidth(), scrollDuration);
        }
        if (scrollY != 0) {
            scrollY = mCallback.interpolateOutOfBoundsScroll(mRecyclerTmView,
                    mSelected.itemView.getHeight(), scrollY,
                    mRecyclerTmView.getHeight(), scrollDuration);
        }
        if (scrollX != 0 || scrollY != 0) {
            if (mDragScrollStartTimeInMs == Long.MIN_VALUE) {
                mDragScrollStartTimeInMs = now;
            }
            mRecyclerTmView.scrollBy(scrollX, scrollY);
            return true;
        }
        mDragScrollStartTimeInMs = Long.MIN_VALUE;
        return false;
    ...
    private List<ViewHolder> findSwapTargets(ViewHolder viewHolder) {
        ...
        final RecyclerTmView.LayoutManager lm = mRecyclerTmView.getLayoutManager();
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            final ViewHolder otherVh = mRecyclerTmView.getChildViewHolder(other);
            if (mCallback.canDropOver(mRecyclerTmView, mSelected, otherVh)) {
    ...
    void moveIfNecessary(ViewHolder viewHolder) {
        if (mRecyclerTmView.isLayoutRequested()) {
            return;
        }
        ...
        if (mCallback.onMove(mRecyclerTmView, viewHolder, target)) {
            // keep target visible
            mCallback.onMoved(mRecyclerTmView, viewHolder, fromPosition,
    ...
    public void onChildViewDetachedFromWindow(@NonNull View view) {
        ...
        final ViewHolder holder = mRecyclerTmView.getChildViewHolder(view);
        if (holder == null) {
            return;
        }
        if (mSelected != null && holder == mSelected) {
            ...
            if (mPendingCleanup.remove(holder.itemView)) {
                mCallback.clearView(mRecyclerTmView, holder);
            }
        }
    }
    ...
    @Override
    public void getItemOffsets(Rect outRect, View view, RecyclerTmView parent,
                               RecyclerTmView.State state) {
        outRect.setEmpty();
    }
    ...
    private ViewHolder findSwipedView(MotionEvent motionEvent) {
        ...
        return mRecyclerTmView.getChildViewHolder(child);
    }
    ...
    void checkSelectForSwipe(int action, MotionEvent motionEvent, int pointerIndex) {
        ...
        if (mRecyclerTmView.getScrollState() == RecyclerTmView.SCROLL_STATE_DRAGGING) {
            return;
        }
        ...
        final int movementFlags = mCallback.getAbsoluteMovementFlags(mRecyclerTmView, vh);
    ...
    View findChildView(MotionEvent event) {
        ...
        return mRecyclerTmView.findChildViewUnder(x, y);
    }
    ...
    public void startDrag(@NonNull ViewHolder viewHolder) {
        if (!mCallback.hasDragFlag(mRecyclerTmView, viewHolder)) {
            Log.e(TAG, "Start drag has been called but dragging is not enabled");
            return;
        }
        if (viewHolder.itemView.getParent() != mRecyclerTmView) {
    ...
    public void startSwipe(@NonNull ViewHolder viewHolder) {
        if (!mCallback.hasSwipeFlag(mRecyclerTmView, viewHolder)) {
    ...
    private int swipeIfNecessary(ViewHolder viewHolder) {
        ...
        final int originalMovementFlags = mCallback.getMovementFlags(mRecyclerTmView, viewHolder);
        final int absoluteMovementFlags = mCallback.convertToAbsoluteDirection(
                originalMovementFlags,
                ViewCompat.getLayoutDirection(mRecyclerTmView));
    ...
    public int checkHorizontalSwipe(ViewHolder viewHolder, int flags) {
        if ((flags & (LEFT | RIGHT)) != 0) {
            ...
            final float threshold = mRecyclerTmView.getWidth() * mCallback
    ...
    private int checkVerticalSwipe(ViewHolder viewHolder, int flags) {
        if ((flags & (UP | DOWN)) != 0) {
            ...
            final float threshold = mRecyclerTmView.getHeight() * mCallback
                    .getSwipeThreshold(viewHolder);
    ...
    private void addChildDrawingOrderCallback() {
        ...
        if (mChildDrawingOrderCallback == null) {
            mChildDrawingOrderCallback = new RecyclerTmView.ChildDrawingOrderCallback() {
                @Override
                public int onGetChildDrawingOrder(int childCount, int i) {
                    ...
                    if (childPosition == -1) {
                        childPosition = mRecyclerTmView.indexOfChild(mOverdrawChild);
        ...
        mRecyclerTmView.setChildDrawingOrderCallback(mChildDrawingOrderCallback);
    }
    ...
    void removeChildDrawingOrderCallbackIfNecessary(View view) {
        if (view == mOverdrawChild) {
            ...
            if (mChildDrawingOrderCallback != null) {
                mRecyclerTmView.setChildDrawingOrderCallback(null);
            }
        }
    }
    ...
    public interface ViewDropHandler {
         ...
         * operation.
         * <p>
         * For example, LinearLayoutTmManager sets up a "scrollToPositionWithOffset" calls so that
     ...
     * {@link #getMovementFlags(RecyclerTmView, ViewHolder)} and return appropriate set
     ...
     * {@link Callback#onMove(RecyclerTmView, ViewHolder, ViewHolder)
     ...
     * in your adapter and also call {@link RecyclerTmView.Adapter#notifyItemMoved(int, int)}.
     * To control where a View can be dropped, you can override
     * {@link #canDropOver(RecyclerTmView, ViewHolder, ViewHolder)}. When a
    ...
    public abstract static class Callback {
         ...
         * {@link RecyclerTmView.ViewHolder#itemView}.
         * <p>
         ...
         * swiping.
         *
         * @param recyclerTmView The RecyclerView to which ItemTouchHelper is attached.
         * @param viewHolder   The ViewHolder for which the movement information is necessary.
         * @return flags specifying which movements are allowed on this ViewHolder.
         * @see #makeMovementFlags(int, int)
         * @see #makeFlag(int, int)
         */
        public abstract int getMovementFlags(@NonNull RecyclerTmView recyclerTmView,
        ...
        final int getAbsoluteMovementFlags(RecyclerTmView recyclerTmView,
                                           ViewHolder viewHolder) {
            final int flags = getMovementFlags(recyclerTmView, viewHolder);
            return convertToAbsoluteDirection(flags, ViewCompat.getLayoutDirection(recyclerTmView));
        }

        boolean hasDragFlag(RecyclerTmView recyclerTmView, ViewHolder viewHolder) {
            final int flags = getAbsoluteMovementFlags(recyclerTmView, viewHolder);
            return (flags & ACTION_MODE_DRAG_MASK) != 0;
        }

        boolean hasSwipeFlag(RecyclerTmView recyclerTmView,
                             ViewHolder viewHolder) {
            final int flags = getAbsoluteMovementFlags(recyclerTmView, viewHolder);
         ...
         *
         * @param recyclerTmView The RecyclerView to which ItemTouchHelper is attached to.
         ...
         */
        @SuppressWarnings("WeakerAccess")
        public boolean canDropOver(@NonNull RecyclerTmView recyclerTmView, @NonNull ViewHolder current,
         ...
         * {@code target}.
         * @see #onMoved(RecyclerTmView, ViewHolder, int, ViewHolder, int, int, int)
         */
        public abstract boolean onMove(@NonNull RecyclerTmView recyclerTmView,
         ...
         *                    {@link RecyclerTmView.ItemDecoration}s.
         ...
         * {@link #getMovementFlags(RecyclerTmView, ViewHolder)} method, this method
         ...
         * {@link #clearView(RecyclerTmView, ViewHolder)}.
         *
         * @param viewHolder The ViewHolder which has been swiped by the user.
         * @param direction  The direction to which the ViewHolder is swiped. It is one of
         *                   {@link #UP}, {@link #DOWN},
         *                   {@link #LEFT} or {@link #RIGHT}. If your
         *                   {@link #getMovementFlags(RecyclerTmView, ViewHolder)}
         *                   method
         ...
         * @see #clearView(RecyclerTmView, RecyclerTmView.ViewHolder)
         */
        ...
        private int getMaxDragScroll(RecyclerTmView recyclerTmView) {
        ...
        /**
         * Called when {@link #onMove(RecyclerTmView, ViewHolder, ViewHolder)} returns true.
         * <p>
         ...
         * keep the View in visible area. For example, for LinearLayoutTmManager, this is as simple
         * as calling {@link LinearLayoutTmManager#scrollToPositionWithOffset(int, int)}.
         *
         * Default implementation calls {@link RecyclerTmView#scrollToPosition(int)} if the View's
         ...
         *
         * @param recyclerTmView The RecyclerView controlled by the ItemTouchHelper.
         ...
         *                     {@link RecyclerTmView.ItemDecoration}s.
         */
        public void onMoved(@NonNull final RecyclerTmView recyclerTmView,
                @NonNull final ViewHolder viewHolder, int fromPos, @NonNull final ViewHolder target,
                int toPos, int x, int y) {
            final RecyclerTmView.LayoutManager layoutManager = recyclerTmView.getLayoutManager();
            ...
            if (layoutManager.canScrollHorizontally()) {
                ...
                if (minLeft <= recyclerTmView.getPaddingLeft()) {
                    recyclerTmView.scrollToPosition(toPos);
                }
                final int maxRight = layoutManager.getDecoratedRight(target.itemView);
                if (maxRight >= recyclerTmView.getWidth() - recyclerTmView.getPaddingRight()) {
                    recyclerTmView.scrollToPosition(toPos);
                }
            }
            ...
            if (layoutManager.canScrollVertically()) {
                ...
                if (minTop <= recyclerTmView.getPaddingTop()) {
                    recyclerTmView.scrollToPosition(toPos);
                }
                final int maxBottom = layoutManager.getDecoratedBottom(target.itemView);
                if (maxBottom >= recyclerTmView.getHeight() - recyclerTmView.getPaddingBottom()) {
                    recyclerTmView.scrollToPosition(toPos);
                }
            }
        }

        void onDraw(Canvas c, RecyclerTmView parent, ViewHolder selected,
        ...
        void onDrawOver(Canvas c, RecyclerTmView parent, ViewHolder selected,
                        List<ItemTouchHelper.RecoverAnimation> recoverAnimationList,
                        int actionState, float dX, float dY) {
            final int recoverAnimSize = recoverAnimationList.size();
            for (int i = 0; i < recoverAnimSize; i++) {
         ...
         * {@link #onSelectedChanged(RecyclerTmView.ViewHolder, int)},
         * {@link #onChildDraw(Canvas, RecyclerTmView, ViewHolder, float, float, int,
         * boolean)} or
         * {@link #onChildDrawOver(Canvas, RecyclerTmView, ViewHolder, float, float, int, boolean)}.
         *
         * @param recyclerTmView The RecyclerView which is controlled by the ItemTouchHelper.
         * @param viewHolder   The View that was interacted by the user.
         */
        public void clearView(@NonNull RecyclerTmView recyclerTmView, @NonNull ViewHolder viewHolder) {
         ...
         * @see #onChildDrawOver(Canvas, RecyclerTmView, ViewHolder, float, float, int,
         * boolean)
         */
        public void onChildDraw(@NonNull Canvas c, @NonNull RecyclerTmView recyclerTmView,
                @NonNull ViewHolder viewHolder,
                float dX, float dY, int actionState, boolean isCurrentlyActive) {
            ItemTouchUIUtilImpl.INSTANCE.onDraw(c, recyclerTmView, viewHolder.itemView, dX, dY,
                    actionState, isCurrentlyActive);
        }
         ...
         * boolean)
         */
        public void onChildDrawOver(@NonNull Canvas c, @NonNull RecyclerTmView recyclerTmView,
                ViewHolder viewHolder,
                float dX, float dY, int actionState, boolean isCurrentlyActive) {
            ItemTouchUIUtilImpl.INSTANCE.onDrawOver(c, recyclerTmView, viewHolder.itemView, dX, dY,
                    actionState, isCurrentlyActive);
        }
         ...
         * {@link RecyclerTmView.ItemAnimator#getMoveDuration()}, otherwise, it returns
         * {@link RecyclerTmView.ItemAnimator#getRemoveDuration()}. If RecyclerView does not have
         * any {@link RecyclerTmView.ItemAnimator} attached, this method returns
         * {@code DEFAULT_DRAG_ANIMATION_DURATION} or {@code DEFAULT_SWIPE_ANIMATION_DURATION}
         * depending on the animation type.
         *
         * @param recyclerTmView  The RecyclerView to which the ItemTouchHelper is attached to.
         ...
         */
        @SuppressWarnings("WeakerAccess")
        public long getAnimationDuration(@NonNull RecyclerTmView recyclerTmView, int animationType,
                                         float animateDx, float animateDy) {
            final RecyclerTmView.ItemAnimator itemAnimator = recyclerTmView.getItemAnimator();
         ...
         *
         * @param recyclerTmView        The RecyclerView instance to which ItemTouchHelper is
         *                            attached to.
         ...
         * be passed to {@link RecyclerTmView#scrollBy(int, int)} method.
         */
        @SuppressWarnings("WeakerAccess")
        public int interpolateOutOfBoundsScroll(@NonNull RecyclerTmView recyclerTmView,
                int viewSize, int viewSizeOutOfBounds,
                int totalSize, long msSinceStartScroll) {
            final int maxScroll = getMaxDragScroll(recyclerTmView);
    ...
    public abstract static class SimpleCallback extends Callback {
         ...
         * {@link #getSwipeDirs(RecyclerTmView, ViewHolder)}
         * and / or {@link #getDragDirs(RecyclerTmView, ViewHolder)}.
         *
         ...
         */
        @SuppressWarnings("WeakerAccess")
        public int getSwipeDirs(@SuppressWarnings("unused") @NonNull RecyclerTmView recyclerTmView,
         ...
         */
        @SuppressWarnings("WeakerAccess")
        public int getDragDirs(@SuppressWarnings("unused") @NonNull RecyclerTmView recyclerTmView,
                @SuppressWarnings("unused") @NonNull ViewHolder viewHolder) {
            return mDefaultDragDirs;
        }

        @Override
        public int getMovementFlags(@NonNull RecyclerTmView recyclerTmView,
                @NonNull ViewHolder viewHolder) {
            return makeMovementFlags(getDragDirs(recyclerTmView, viewHolder),
                    getSwipeDirs(recyclerTmView, viewHolder));
        }
    }
    ...
    private class ItemTouchHelperGestureListener extends GestureDetector.SimpleOnGestureListener {
        ...
        public void onLongPress(MotionEvent e) {
            ...
            if (child != null) {
                ViewHolder vh = mRecyclerTmView.getChildViewHolder(child);
                if (vh != null) {
                    if (!mCallback.hasDragFlag(mRecyclerTmView, vh)) {
                        return;
                    }
