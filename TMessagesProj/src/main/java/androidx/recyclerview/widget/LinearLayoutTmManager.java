...
/**
 * A {@link RecyclerTmView.LayoutManager} implementation which provides
 * similar functionality to {@link android.widget.ListView}.
 */
public class LinearLayoutTmManager extends RecyclerTmView.LayoutManager implements
        ItemTouchHelper.ViewDropHandler, RecyclerTmView.SmoothScroller.ScrollVectorProvider {

    private static final String TAG = "LinearLayoutTmManager";

    static final boolean DEBUG = false;

    public static final int HORIZONTAL = RecyclerTmView.HORIZONTAL;

    public static final int VERTICAL = RecyclerTmView.VERTICAL;
     ...
     */
    @RecyclerTmView.Orientation
    int mOrientation = RecyclerTmView.DEFAULT_ORIENTATION;
     ...
     * helps {@link LinearLayoutTmManager} make those decisions.
     */
     ...
     * {@link #onLayoutChildren(RecyclerTmView.Recycler, RecyclerTmView.State)} is run.
     */
     ...
     */
    int mPendingScrollPosition = RecyclerTmView.NO_POSITION;
    ...
    /**
     * Creates a vertical LinearLayoutTmManager
     *
     * @param context Current context, will be used to access resources.
     */
    public LinearLayoutTmManager(Context context) {
        this(context, RecyclerTmView.DEFAULT_ORIENTATION, false);
    }
     ...
     */
    public LinearLayoutTmManager(Context context, @RecyclerTmView.Orientation int orientation,
     ...
     */
    @Override
    public RecyclerTmView.LayoutParams generateDefaultLayoutParams() {
        return new RecyclerTmView.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT,
     ...
     * RecyclerView.
     * <p>
     * If you are using a {@link RecyclerTmView.RecycledViewPool}, it might be a good idea to set
    ...
    @Override
    public void onDetachedFromWindow(RecyclerTmView view, RecyclerTmView.Recycler recycler) {
    ...
    /**
     * Sets the orientation of the layout. {@link LinearLayoutTmManager}
     * will do its best to keep scroll position.
     *
     * @param orientation {@link #HORIZONTAL} or {@link #VERTICAL}
     */
    public void setOrientation(@RecyclerTmView.Orientation int orientation) {
     ...
     * When set to true, If {@link RecyclerTmView} is LTR, than it will
     * layout from RTL, if {@link RecyclerTmView}} is RTL, it will layout
     * from LTR.
     *
     ...
     *
     * <p>By default, {@link LinearLayoutTmManager} lays out 1 extra page
     ...
     * location, where 1) the extra content helps LinearLayoutTmManager know in advance when its
     ...
     * @deprecated  Use {@link #calculateExtraLayoutSpace(RecyclerTmView.State, int[])} instead.
     */
    @SuppressWarnings("DeprecatedIsStillUsed")
    @Deprecated
    protected int getExtraLayoutSpace(RecyclerTmView.State state) {
     ...
     * LinearLayoutTmManager} and stores the result in {@code extraLayoutSpace}. {@code
     ...
     *
     * <p>By default, {@code LinearLayoutTmManager} lays out 1 extra page of items while smooth
     ...
     * {@link RecyclerTmView.State#hasTargetScrollPosition()} to find out if a smooth scroll to a
     * position is in progress, and {@link RecyclerTmView.State#getTargetScrollPosition()} to find out
     ...
     * where 1) the extra content helps LinearLayoutTmManager know in advance when its target is
     ...
     */
    protected void calculateExtraLayoutSpace(@NonNull RecyclerTmView.State state,
    ...
    @Override
    public void smoothScrollToPosition(RecyclerTmView recyclerTmView, RecyclerTmView.State state,
                                       int position) {
        LinearSmoothScroller linearSmoothScroller =
                new LinearSmoothScroller(recyclerTmView.getContext());
     ...
     */
    @Override
    public void onLayoutChildren(RecyclerTmView.Recycler recycler, RecyclerTmView.State state) {
        // layout algorithm:
        ...
        if (mPendingSavedState != null || mPendingScrollPosition != RecyclerTmView.NO_POSITION) {
        ...
        if (!mAnchorInfo.mValid || mPendingScrollPosition != RecyclerTmView.NO_POSITION
        ...
        if (state.isPreLayout() && mPendingScrollPosition != RecyclerTmView.NO_POSITION
    ...
    @Override
    public void onLayoutCompleted(RecyclerTmView.State state) {
        super.onLayoutCompleted(state);
        mPendingSavedState = null; // we don't need this anymore
        mPendingScrollPosition = RecyclerTmView.NO_POSITION;
        mPendingScrollPositionOffset = INVALID_OFFSET;
        mAnchorInfo.reset();
    }
     ...
     *                                 indices.
     */
    void onAnchorReady(RecyclerTmView.Recycler recycler, RecyclerTmView.State state,
     ...
     */
    private void layoutForPredictiveAnimations(RecyclerTmView.Recycler recycler,
                                               RecyclerTmView.State state, int startOffset,
                                               int endOffset) {
        ...
        final List<RecyclerTmView.ViewHolder> scrapList = recycler.getScrapList();
        final int scrapSize = scrapList.size();
        final int firstChildPos = getPosition(getChildAt(0));
        for (int i = 0; i < scrapSize; i++) {
            RecyclerTmView.ViewHolder scrap = scrapList.get(i);
            if (scrap.isRemoved()) {
                continue;
            }
    ...
    private void updateAnchorInfoForLayout(RecyclerTmView.Recycler recycler, RecyclerTmView.State state,
     ...
     */
    private boolean updateAnchorFromChildren(RecyclerTmView.Recycler recycler,
                                             RecyclerTmView.State state, AnchorInfo anchorInfo) {
        if (getChildCount() == 0) {
            return false;
        }
        final View focused = getFocusedChild();
     ...
     */
    private boolean updateAnchorFromPendingData(RecyclerTmView.State state, AnchorInfo anchorInfo) {
        if (state.isPreLayout() || mPendingScrollPosition == RecyclerTmView.NO_POSITION) {
            return false;
        }
     ...
     */
    private int fixLayoutEndGap(int endOffset, RecyclerTmView.Recycler recycler,
                                RecyclerTmView.State state, boolean canOffsetChildren) {
     ...
     * @return The final offset amount for children
     */
    private int fixLayoutStartGap(int startOffset, RecyclerTmView.Recycler recycler,
                                  RecyclerTmView.State state, boolean canOffsetChildren) {
     ...
     */
    @Override
    public int scrollHorizontallyBy(int dx, RecyclerTmView.Recycler recycler,
     ...
     */
    @Override
    public int scrollVerticallyBy(int dy, RecyclerTmView.Recycler recycler,
                                  RecyclerTmView.State state) {
        if (mOrientation == HORIZONTAL) {
            return 0;
        }
    ...
    @Override
    public int computeHorizontalScrollOffset(RecyclerTmView.State state) {
        return computeScrollOffset(state);
    }

    @Override
    public int computeVerticalScrollOffset(RecyclerTmView.State state) {
        return computeScrollOffset(state);
    }

    @Override
    public int computeHorizontalScrollExtent(RecyclerTmView.State state) {
        return computeScrollExtent(state);
    }

    @Override
    public int computeVerticalScrollExtent(RecyclerTmView.State state) {
        return computeScrollExtent(state);
    }

    @Override
    public int computeHorizontalScrollRange(RecyclerTmView.State state) {
        return computeScrollRange(state);
    }

    @Override
    public int computeVerticalScrollRange(RecyclerTmView.State state) {
        return computeScrollRange(state);
    }

    private int computeScrollOffset(RecyclerTmView.State state) {
        if (getChildCount() == 0) {
            return 0;
        }
        ensureLayoutState();
    ...
    private int computeScrollExtent(RecyclerTmView.State state) {
        if (getChildCount() == 0) {
            return 0;
        }
        ensureLayoutState();
    ...
    private int computeScrollRange(RecyclerTmView.State state) {
        if (getChildCount() == 0) {
            return 0;
        }
        ensureLayoutState();
    ...
    private void updateLayoutState(int layoutDirection, int requiredSpace,
                                   boolean canUseExistingSpace, RecyclerTmView.State state) {
    ...
    void collectPrefetchPositionsForLayoutState(RecyclerTmView.State state, LayoutState layoutState,
    ...
    public void collectInitialPrefetchPositions(int adapterItemCount,
        ...
        if (mPendingSavedState != null && mPendingSavedState.hasValidAnchor()) {
            ...
            if (mPendingScrollPosition == RecyclerTmView.NO_POSITION) {
     ...
     * <code>4</code> to this method for each inner RecyclerView's LinearLayoutTmManager will enable
    ...
    @Override
    public void collectAdjacentPrefetchPositions(int dx, int dy, RecyclerTmView.State state,
                                                 LayoutPrefetchRegistry layoutPrefetchRegistry) {
    ...
    int scrollBy(int delta, RecyclerTmView.Recycler recycler, RecyclerTmView.State state) {
        if (getChildCount() == 0 || delta == 0) {
            return 0;
        }
        ensureLayoutState();
     ...
     */
    protected void recycleChildren(RecyclerTmView.Recycler recycler, int startIndex, int endIndex) {
     ...
     *
     * @param recycler Recycler instance of {@link RecyclerTmView}
     ...
     */
    protected void recycleViewsFromStart(RecyclerTmView.Recycler recycler, int scrollingOffset,
                                         int noRecycleSpace) {
     ...
     *                       #calculateExtraLayoutSpace}.
     */
    private void recycleViewsFromEnd(RecyclerTmView.Recycler recycler, int scrollingOffset,
                                     int noRecycleSpace) {
        final int childCount = getChildCount();
        if (scrollingOffset < 0) {
            if (DEBUG) {
     ...
     * @see #recycleViewsFromStart(RecyclerTmView.Recycler, int, int)
     * @see #recycleViewsFromEnd(RecyclerTmView.Recycler, int, int)
     * @see LinearLayoutTmManager.LayoutState#mLayoutDirection
     */
    private void recycleByLayoutState(RecyclerTmView.Recycler recycler, LayoutState layoutState) {
     ...
     * independent from the rest of the {@link LinearLayoutTmManager}
     ...
     */
    int fill(RecyclerTmView.Recycler recycler, LayoutState layoutState,
             RecyclerTmView.State state, boolean stopOnFocusable) {
        ...
        while ((layoutState.mInfinite || remainingSpace > 0) && layoutState.hasMore(state)) {
            ...
            if (RecyclerTmView.VERBOSE_TRACING) {
    ...
    void layoutChunk(RecyclerTmView.Recycler recycler, RecyclerTmView.State state,
        ...
        RecyclerTmView.LayoutParams params = (RecyclerTmView.LayoutParams) view.getLayoutParams();
     ...
     */
    private View findReferenceChildClosestToEnd(RecyclerTmView.Recycler recycler,
                                                RecyclerTmView.State state) {
     ...
     * @return A View that can be used an an anchor View.
     */
    private View findReferenceChildClosestToStart(RecyclerTmView.Recycler recycler,
                                                  RecyclerTmView.State state) {
        return mShouldReverseLayout ? findLastReferenceChild(recycler, state) :
                findFirstReferenceChild(recycler, state);
    }

    private View findFirstReferenceChild(RecyclerTmView.Recycler recycler, RecyclerTmView.State state) {
        return findReferenceChild(recycler, state, 0, getChildCount(), state.getItemCount());
    }

    private View findLastReferenceChild(RecyclerTmView.Recycler recycler, RecyclerTmView.State state) {
        return findReferenceChild(recycler, state, getChildCount() - 1, -1, state.getItemCount());
    }

    // overridden by GridLayoutManager
    View findReferenceChild(RecyclerTmView.Recycler recycler, RecyclerTmView.State state,
        ...
        for (int i = start; i != end; i += diff) {
            ...
            if (position >= 0 && position < itemCount) {
                if (((RecyclerTmView.LayoutParams) view.getLayoutParams()).isItemRemoved()) {
     ...
     *
     * @return The adapter position of the first visible item or {@link RecyclerTmView#NO_POSITION} if
    ...
    public int findFirstVisibleItemPosition() {
        ...
        return child == null ? RecyclerTmView.NO_POSITION : getPosition(child);
    }
     ...
     * {@link RecyclerTmView#NO_POSITION} if there aren't any visible items.
     ...
     * are ignored in this method.
     *
     * @return The adapter position of the last visible view or {@link RecyclerTmView#NO_POSITION} if
     * there aren't any visible items.
     * @see #findLastCompletelyVisibleItemPosition()
     * @see #findFirstVisibleItemPosition()
     */
    ...
    public View onFocusSearchFailed(View focused, int focusDirection,
                                    RecyclerTmView.Recycler recycler, RecyclerTmView.State state) {
        resolveShouldLayoutReverse();
        if (getChildCount() == 0) {
            return null;
        }
    ...
    static class LayoutState {
        ...
        /**
         * Contains the {@link #calculateExtraLayoutSpace(RecyclerTmView.State, int[])}  extra layout
        ...
        /**
         * Equal to {@link RecyclerTmView.State#isPreLayout()}. When consuming scrap, if this value
        ...
        /**
         * The most recent {@link #scrollBy(int, RecyclerTmView.Recycler, RecyclerTmView.State)}
         * amount.
         */
         ...
         */
        List<RecyclerTmView.ViewHolder> mScrapList = null;
         ...
         */
        boolean hasMore(RecyclerTmView.State state) {
         ...
         */
        View next(RecyclerTmView.Recycler recycler) {
            if (mScrapList != null) {
        ...
        private View nextViewFromScrapList() {
            ...
            for (int i = 0; i < size; i++) {
                ...
                final RecyclerTmView.LayoutParams lp = (RecyclerTmView.LayoutParams) view.getLayoutParams();
                if (lp.isItemRemoved()) {
                    continue;
                }
        ...
        public void assignPositionFromScrapList(View ignore) {
            ...
            if (closest == null) {
                mCurrentPosition = RecyclerTmView.NO_POSITION;
            } else {
                mCurrentPosition = ((RecyclerTmView.LayoutParams) closest.getLayoutParams())
    ...
    public static class SavedState implements Parcelable {
        ...
        void invalidateAnchor() {
            mAnchorPosition = RecyclerTmView.NO_POSITION;
        }
    ...
    static class AnchorInfo {
        ...
        void reset() {
            mPosition = RecyclerTmView.NO_POSITION;
        ...
        boolean isViewValidAsAnchor(View child, RecyclerTmView.State state) {
            RecyclerTmView.LayoutParams lp = (RecyclerTmView.LayoutParams) child.getLayoutParams();
