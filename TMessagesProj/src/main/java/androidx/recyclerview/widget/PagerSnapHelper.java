 ...
 * {@link androidx.viewpager.widget.ViewPager}. Set both {@link RecyclerTmView} and the items of the
 * {@link RecyclerTmView.Adapter} to have
 * {@link android.view.ViewGroup.LayoutParams#MATCH_PARENT} height and width and then attach
 * PagerSnapHelper to the {@link RecyclerTmView} using {@link #attachToRecyclerView(RecyclerTmView)}.
 */
public class PagerSnapHelper extends SnapHelper {
    ...
    @Nullable
    @Override
    public int[] calculateDistanceToFinalSnap(@NonNull RecyclerTmView.LayoutManager layoutManager,
    ...
    @Nullable
    @Override
    public View findSnapView(RecyclerTmView.LayoutManager layoutManager) {
        if (layoutManager.canScrollVertically()) {
    ...
    @Override
    public int findTargetSnapPosition(RecyclerTmView.LayoutManager layoutManager, int velocityX,
                                      int velocityY) {
    ...
    private boolean isForwardFling(RecyclerTmView.LayoutManager layoutManager, int velocityX,
                                   int velocityY) {
        if (layoutManager.canScrollHorizontally()) {
            return velocityX > 0;
        } else {
    ...
    private boolean isReverseLayout(RecyclerTmView.LayoutManager layoutManager) {
        final int itemCount = layoutManager.getItemCount();
        if ((layoutManager instanceof RecyclerTmView.SmoothScroller.ScrollVectorProvider)) {
            RecyclerTmView.SmoothScroller.ScrollVectorProvider vectorProvider =
                    (RecyclerTmView.SmoothScroller.ScrollVectorProvider) layoutManager;
    ...
    private int distanceToCenter(@NonNull RecyclerTmView.LayoutManager layoutManager,
     ...
     *                      {@link RecyclerTmView}.
     * @param helper The relevant {@link OrientationHelper} for the attached {@link RecyclerTmView}.
     *
     * @return the child view that is currently closest to the center of this parent.
     */
    @Nullable
    private View findCenterView(RecyclerTmView.LayoutManager layoutManager,
    ...
    @Nullable
    private OrientationHelper getOrientationHelper(RecyclerTmView.LayoutManager layoutManager) {
        if (layoutManager.canScrollVertically()) {
    ...
    @NonNull
    private OrientationHelper getVerticalHelper(@NonNull RecyclerTmView.LayoutManager layoutManager) {
    ...
    private OrientationHelper getHorizontalHelper(
            @NonNull RecyclerTmView.LayoutManager layoutManager) {
