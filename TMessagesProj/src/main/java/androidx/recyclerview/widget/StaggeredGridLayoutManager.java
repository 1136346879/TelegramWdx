 ...
 */
public class StaggeredGridLayoutManager extends RecyclerTmView.LayoutManager implements
        RecyclerTmView.SmoothScroller.ScrollVectorProvider {
    ...
    /**
     * When scroll state is changed to {@link RecyclerTmView#SCROLL_STATE_IDLE}, StaggeredGrid will
    ...
    public void onScrollStateChanged(int state) {
        if (state == RecyclerTmView.SCROLL_STATE_IDLE) {
            checkForGaps();
        }
    }
     ...
     * When set to true, If {@link RecyclerTmView} is LTR, than it will layout from RTL, if
     * {@link RecyclerTmView}} is RTL, it will layout from LTR.
     *
    ...
    private void onLayoutChildren(RecyclerTmView.Recycler recycler, RecyclerTmView.State state,
        ...
        boolean recalculateAnchor = !anchorInfo.mValid || mPendingScrollPosition != RecyclerTmView.NO_POSITION
    ...
    private void applyPendingSavedState(AnchorInfo anchorInfo) {
        ...
        if (mPendingSavedState.mAnchorPosition != RecyclerTmView.NO_POSITION) {
    ...
    void updateAnchorInfoForLayout(RecyclerTmView.State state, AnchorInfo anchorInfo) {
    ...
    private boolean updateAnchorFromChildren(RecyclerTmView.State state, AnchorInfo anchorInfo) {
    ...
    boolean updateAnchorFromPendingData(RecyclerTmView.State state, AnchorInfo anchorInfo) {
        ...
        if (mPendingSavedState == null || mPendingSavedState.mAnchorPosition == RecyclerTmView.NO_POSITION
                || mPendingSavedState.mSpanOffsetsSize < 1) {
     ...
     * any items, {@link RecyclerTmView#NO_POSITION} is returned for that span.
     ...
     * not have any items, {@link RecyclerTmView#NO_POSITION} is returned for that span.
     ...
     * have any items, {@link RecyclerTmView#NO_POSITION} is returned for that span.
     * @see #findFirstCompletelyVisibleItemPositions(int[])
     * @see #findLastVisibleItemPositions(int[])
     */
    ...
    public Parcelable onSaveInstanceState() {
        ...
        if (getChildCount() > 0) {
            ...
            state.mAnchorPosition = RecyclerTmView.NO_POSITION;
            state.mVisibleAnchorPosition = RecyclerTmView.NO_POSITION;
    ...
    @Override
    public void onInitializeAccessibilityNodeInfoForItem(RecyclerTmView.Recycler recycler,
                                                         RecyclerTmView.State state, View host, AccessibilityNodeInfoCompat info) {
    ...
    int findFirstVisibleItemPositionInt() {
        ...
        return first == null ? RecyclerTmView.NO_POSITION : getPosition(first);
    }

    @Override
    public int getRowCountForAccessibility(RecyclerTmView.Recycler recycler,
    ...
    @Override
    public int getColumnCountForAccessibility(RecyclerTmView.Recycler recycler,
                                              RecyclerTmView.State state) {
        if (mOrientation == VERTICAL) {
            return mSpanCount;
        }
    ...
    private void fixEndGap(RecyclerTmView.Recycler recycler, RecyclerTmView.State state,
    ...
    private void fixStartGap(RecyclerTmView.Recycler recycler, RecyclerTmView.State state,
                             boolean canOffsetChildren) {
    ...
    private void updateLayoutState(int anchorPosition, RecyclerTmView.State state) {
        ...
        if (isSmoothScrolling()) {
            ...
            if (targetPos != RecyclerTmView.NO_POSITION) {
    ...
    @Override
    public void onItemsRemoved(RecyclerTmView recyclerTmView, int positionStart, int itemCount) {
        handleUpdate(positionStart, itemCount, AdapterHelper.UpdateOp.REMOVE);
    }

    @Override
    public void onItemsAdded(RecyclerTmView recyclerTmView, int positionStart, int itemCount) {
        handleUpdate(positionStart, itemCount, AdapterHelper.UpdateOp.ADD);
    }

    @Override
    public void onItemsChanged(RecyclerTmView recyclerTmView) {
        mLazySpanLookup.clear();
        requestLayout();
    }

    @Override
    public void onItemsMoved(RecyclerTmView recyclerTmView, int from, int to, int itemCount) {
        handleUpdate(from, to, AdapterHelper.UpdateOp.MOVE);
    }

    @Override
    public void onItemsUpdated(RecyclerTmView recyclerTmView, int positionStart, int itemCount,
                               Object payload) {
    ...
    private int fill(RecyclerTmView.Recycler recycler, LayoutState layoutState,
                     RecyclerTmView.State state) {
    ...
    private void recycle(RecyclerTmView.Recycler recycler, LayoutState layoutState) {
    ...
    private void recycleFromStart(RecyclerTmView.Recycler recycler, int line) {
    ...
    private void recycleFromEnd(RecyclerTmView.Recycler recycler, int line) {
        final int childCount = getChildCount();
        int i;
    ...
    public void smoothScrollToPosition(RecyclerTmView recyclerTmView, RecyclerTmView.State state,
                                       int position) {
        LinearSmoothScroller scroller = new LinearSmoothScroller(recyclerTmView.getContext());
    ...
    void prepareLayoutStateForDelta(int delta, RecyclerTmView.State state) {
    ...
    int scrollBy(int dt, RecyclerTmView.Recycler recycler, RecyclerTmView.State state) {
    ...
    @Override
    public RecyclerTmView.LayoutParams generateLayoutParams(Context c, AttributeSet attrs) {
        return new LayoutParams(c, attrs);
    }

    @Override
    public RecyclerTmView.LayoutParams generateLayoutParams(ViewGroup.LayoutParams lp) {
    ...
    @Override
    public boolean checkLayoutParams(RecyclerTmView.LayoutParams lp) {
    ...
    @Nullable
    @Override
    public View onFocusSearchFailed(View focused, int direction, RecyclerTmView.Recycler recycler,
                                    RecyclerTmView.State state) {
        if (getChildCount() == 0) {
            return null;
        }
     ...
     */
    public static class LayoutParams extends RecyclerTmView.LayoutParams {
        ...
        public LayoutParams(RecyclerTmView.LayoutParams source) {
            super(source);
        }
    ...
    class Span {
         ...
         * {@link #onFocusSearchFailed(View, int, RecyclerTmView.Recycler, RecyclerTmView.State)} for
         ...
         * {@link RecyclerTmView#NO_POSITION} if no such view is found.
         */
    ...
    static class LazySpanLookup {
        ...
        int invalidateAfter(int position) {
            ...
            if (endPosition == RecyclerTmView.NO_POSITION) {
    ...
    public static class SavedState implements Parcelable {
        ...
        void invalidateAnchorPositionInfo() {
            ...
            mVisibleAnchorPosition = RecyclerTmView.NO_POSITION;
        }
