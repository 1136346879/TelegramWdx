...
class ViewInfoStore {
     ...
     */
    @VisibleForTesting
    final ArrayMap<RecyclerTmView.ViewHolder, InfoRecord> mLayoutHolderMap = new ArrayMap<>();

    @VisibleForTesting
    final LongSparseArray<RecyclerTmView.ViewHolder> mOldChangedHolders = new LongSparseArray<>();
     ...
     */
    void addToPreLayout(RecyclerTmView.ViewHolder holder, RecyclerTmView.ItemAnimator.ItemHolderInfo info) {
    ...
    boolean isDisappearing(RecyclerTmView.ViewHolder holder) {
     ...
     */
    @Nullable
    RecyclerTmView.ItemAnimator.ItemHolderInfo popFromPreLayout(RecyclerTmView.ViewHolder vh) {
     ...
     */
    @Nullable
    RecyclerTmView.ItemAnimator.ItemHolderInfo popFromPostLayout(RecyclerTmView.ViewHolder vh) {
        return popFromLayoutStep(vh, FLAG_POST);
    }

    private RecyclerTmView.ItemAnimator.ItemHolderInfo popFromLayoutStep(RecyclerTmView.ViewHolder vh, int flag) {
        ...
        if (record != null && (record.flags & flag) != 0) {
            record.flags &= ~flag;
            final RecyclerTmView.ItemAnimator.ItemHolderInfo info;
            if (flag == FLAG_PRE) {
                info = record.preInfo;
     ...
     */
    void addToOldChangeHolders(long key, RecyclerTmView.ViewHolder holder) {
     ...
     * @param info The information to save
     */
    void addToAppearedInPreLayoutHolders(RecyclerTmView.ViewHolder holder, RecyclerTmView.ItemAnimator.ItemHolderInfo info) {
        InfoRecord record = mLayoutHolderMap.get(holder);
        if (record == null) {
            record = InfoRecord.obtain();
            mLayoutHolderMap.put(holder, record);
     ...
     */
    boolean isInPreLayout(RecyclerTmView.ViewHolder viewHolder) {
     ...
     */
    RecyclerTmView.ViewHolder getFromOldChangeHolders(long key) {
     ...
     * @param info The information to save
     */
    void addToPostLayout(RecyclerTmView.ViewHolder holder, RecyclerTmView.ItemAnimator.ItemHolderInfo info) {
        InfoRecord record = mLayoutHolderMap.get(holder);
        if (record == null) {
            record = InfoRecord.obtain();
            mLayoutHolderMap.put(holder, record);
     ...
     */
    void addToDisappearedInLayout(RecyclerTmView.ViewHolder holder) {
        InfoRecord record = mLayoutHolderMap.get(holder);
        if (record == null) {
            record = InfoRecord.obtain();
            mLayoutHolderMap.put(holder, record);
     ...
     */
    void removeFromDisappearedInLayout(RecyclerTmView.ViewHolder holder) {
        InfoRecord record = mLayoutHolderMap.get(holder);
        if (record == null) {
            return;
        }
    ...
    void process(ProcessCallback callback) {
        for (int index = mLayoutHolderMap.size() - 1; index >= 0; index--) {
            final RecyclerTmView.ViewHolder viewHolder = mLayoutHolderMap.keyAt(index);
     ...
     */
    void removeViewHolder(RecyclerTmView.ViewHolder holder) {
    ...
    public void onViewDetached(RecyclerTmView.ViewHolder viewHolder) {
        removeFromDisappearedInLayout(viewHolder);
    }

    interface ProcessCallback {
        void processDisappeared(RecyclerTmView.ViewHolder viewHolder, @NonNull RecyclerTmView.ItemAnimator.ItemHolderInfo preInfo,
                                @Nullable RecyclerTmView.ItemAnimator.ItemHolderInfo postInfo);
        void processAppeared(RecyclerTmView.ViewHolder viewHolder, @Nullable RecyclerTmView.ItemAnimator.ItemHolderInfo preInfo,
                             RecyclerTmView.ItemAnimator.ItemHolderInfo postInfo);
        void processPersistent(RecyclerTmView.ViewHolder viewHolder, @NonNull RecyclerTmView.ItemAnimator.ItemHolderInfo preInfo,
                               @NonNull RecyclerTmView.ItemAnimator.ItemHolderInfo postInfo);
        void unused(RecyclerTmView.ViewHolder holder);
    }
    ...
    static class InfoRecord {
        ...
        int flags;
        @Nullable
        RecyclerTmView.ItemAnimator.ItemHolderInfo preInfo;
        @Nullable
        RecyclerTmView.ItemAnimator.ItemHolderInfo postInfo;
