...
public class EmojiView extends FrameLayout implements NotificationCenter.NotificationCenterDelegate {
    ...
    public EmojiView(boolean needStickers, boolean needGif, final Context context, boolean needSearch, final TLRPC.ChatFull chatFull) {
        ...
        emojiGridView.setOnScrollListener(new RecyclerTmView.OnScrollListener() {
            @Override
            public void onScrollStateChanged(RecyclerTmView recyclerTmView, int newState) {
                if (newState == RecyclerTmView.SCROLL_STATE_DRAGGING && emojiSearchField != null) {
            ...
            public void onScrolled(RecyclerTmView recyclerTmView, int dx, int dy) {
                ...
                checkEmojiTabY(recyclerTmView, dy);
        ...
        if (needStickers) {
            if (needGif) {
                ...
                gifGridView.addItemDecoration(new RecyclerTmView.ItemDecoration() {
                    @Override
                    public void getItemOffsets(android.graphics.Rect outRect, View view, RecyclerTmView parent, RecyclerTmView.State state) {
                ...
                gifGridView.setOnScrollListener(new RecyclerTmView.OnScrollListener() {
                    @Override
                    public void onScrollStateChanged(RecyclerTmView recyclerTmView, int newState) {
            ...
            trendingGridView.setOnScrollListener(new RecyclerTmView.OnScrollListener() {
                @Override
                public void onScrolled(RecyclerTmView recyclerTmView, int dx, int dy) {
                    checkStickersTabY(recyclerTmView, dy);
                    checkBottomTabScroll(dy);
                }
            });
    ...
    private void checkStickersSearchFieldScroll(boolean isLayout) {
        if (delegate != null && delegate.isSearchOpened()) {
            RecyclerTmView.ViewHolder holder = stickersGridView.findViewHolderForAdapterPosition(0);
            if (holder == null) {
    ...
    private void checkEmojiSearchFieldScroll(boolean isLayout) {
        if (delegate != null && delegate.isSearchOpened()) {
            RecyclerTmView.ViewHolder holder = emojiGridView.findViewHolderForAdapterPosition(0);
            if (holder == null) {
    ...
    private void checkGifSearchFieldScroll(boolean isLayout) {
        if (gifGridView != null && gifGridView.getAdapter() == gifSearchAdapter && !gifSearchAdapter.searchEndReached && gifSearchAdapter.reqId == 0 && !gifSearchAdapter.results.isEmpty()) {
            ...
            if (position != RecyclerTmView.NO_POSITION && position > gifLayoutManager.getItemCount() - 5) {
                gifSearchAdapter.search(gifSearchAdapter.lastSearchImageString, gifSearchAdapter.nextSearchOffset, true);
            }
        }
        if (delegate != null && delegate.isSearchOpened()) {
            RecyclerTmView.ViewHolder holder = gifGridView.findViewHolderForAdapterPosition(0);
            if (holder == null) {
        ...
        public RecyclerTmView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
            ...
            switch (viewType) {
                ...
                case 3:
                    ...
                    view.setLayoutParams(new RecyclerTmView.LayoutParams(LayoutHelper.MATCH_PARENT, LayoutHelper.WRAP_CONTENT));
                    break;
                case 4:
                    view = new View(context);
                    view.setLayoutParams(new RecyclerTmView.LayoutParams(LayoutHelper.MATCH_PARENT, searchFieldHeight));
                    break;
            }
