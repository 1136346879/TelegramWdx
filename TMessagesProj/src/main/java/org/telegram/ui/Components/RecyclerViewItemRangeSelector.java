...
public class RecyclerViewItemRangeSelector implements RecyclerTmView.OnItemTouchListener {

    private RecyclerTmView recyclerTmView;
    ...
    private Runnable autoScrollRunnable = new Runnable() {
        ...
        public void run() {
            ...
            if (inTopHotspot) {
                recyclerTmView.scrollBy(0, -autoScrollVelocity);
    ...
    @Override
    public boolean onInterceptTouchEvent(RecyclerTmView rv, MotionEvent e) {
    ...
    @Override
    public void onTouchEvent(RecyclerTmView rv, MotionEvent e) {
        View v = rv.findChildViewUnder(e.getX(), e.getY());
        int itemPosition;
        if (v != null) {
            itemPosition = rv.getChildAdapterPosition(v);
        } else {
            itemPosition = RecyclerTmView.NO_POSITION;
        }
        float y = e.getY();
        switch (e.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                if (itemPosition != RecyclerTmView.NO_POSITION) {
