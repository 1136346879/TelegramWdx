...
public class MediaActivity extends BaseFragment implements NotificationCenter.NotificationCenterDelegate {
    ...
    public View createView(Context context) {
        ...
        for (int a = 0; a < mediaPages.length; a++) {
            ...
            final LinearLayoutTmManager layoutManager = mediaPages[a].layoutManager = new LinearLayoutTmManager(context, LinearLayoutTmManager.VERTICAL, false) {
                @Override
            ...
            mediaPages[a].listView.setOnScrollListener(new RecyclerTmView.OnScrollListener() {

                @Override
                public void onScrollStateChanged(RecyclerTmView recyclerTmView, int newState) {
                    if (newState == RecyclerTmView.SCROLL_STATE_DRAGGING && searching && searchWas) {
                        AndroidUtilities.hideKeyboard(getParentActivity().getCurrentFocus());
                    }
                    scrolling = newState != RecyclerTmView.SCROLL_STATE_IDLE;
                ...
                public void onScrolled(RecyclerTmView recyclerTmView, int dx, int dy) {
                    ...
                    int visibleItemCount = firstVisibleItem == RecyclerTmView.NO_POSITION ? 0 : Math.abs(layoutManager.findLastVisibleItemPosition() - firstVisibleItem) + 1;
                    ...
                    if (recyclerTmView == mediaPages[0].listView && !searching && !actionBar.isActionModeShowed()) {
                    ...
                    updateSections(recyclerTmView, false);
                }
            });
    ...
    private void switchToCurrentSelectedMode(boolean animated) {
        ...
        int a = animated ? 1 : 0;
        RecyclerTmView.Adapter currentAdapter = mediaPages[a].listView.getAdapter();
        if (searching && searchWas) {
            if (animated) {
    ...
    private void recycleAdapter(RecyclerTmView.Adapter adapter) {
    ...
    private class SharedLinksAdapter extends RecyclerListView.SectionsAdapter {
        ...
        @Override
        public void onBindViewHolder(int section, int position, RecyclerTmView.ViewHolder holder) {
